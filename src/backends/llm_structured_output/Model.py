from chat.RejectedCompletion import RejectedCompletion
from chat.ReusableKVCache import ReusableKVCache
...
class Model:
    ...
    def _evaluate_prompt(
        self,
        prompt: list[int],
        prior_prompt: Optional[list[int]] = None,
        prior_cache=None,
    ):
        if prior_prompt:
            i = 0
    ...
    def generate_without_schema(self, logits, cache, temp: float = 0.0):
        """
    ...
    def generate_with_schema(self, logits, cache, token_acceptor, temp: float = 0.0):
        while True:
